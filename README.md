...
- n8n يستخدم مفتاح تشفير (`N8N_ENCRYPTION_KEY`) لتشفير البيانات الحسّاسة داخل النظام.
- لا تضع المفتاح في المستودع. يمكنك توليد مفتاح قوي محليًا باستخدام:

```bash
openssl rand -base64 32
```

- أضف القيمة الناتجة إلى أسرار المستودع (`Settings → Secrets`) باسم `N8N_ENCRYPTION_KEY` أو ضعها في ملف `.env` محلي قبل التشغيل (ملف `.env` مُدرَج في `.gitignore`).
